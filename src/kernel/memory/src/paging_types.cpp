...
void MdOS::mem::virt::set_type(EntryType type, PageEntry *entry) {
	*entry &= ~uint64_t(0xE00);
	*entry |= uint64_t(type) << 9;
}

MdOS::mem::virt::EntryType MdOS::mem::virt::get_type(PageEntry *entry) {
	return EntryType(uint8_t(*entry >> 9) & uint8_t(0b0111));
}

void MdOS::mem::virt::set_bit(EntryControlBit bit, bool value, PageEntry *entry) {
...
bool MdOS::mem::virt::get_bit(EntryControlBit bit, PageEntry *entry) {
	EntryType type = get_type(entry);
	switch (bit) {
		case EntryControlBit::PagePresent:
			return *entry & 1ULL << 0;
...
void MdOS::mem::virt::set_addr(uint64_t addr, PageEntry *entry) {
	*entry &= ~0x000FFFFFFFFFF000ULL;
	*entry |= (addr & ~0xFFFULL);
}

uint64_t MdOS::mem::virt::get_addr(PageEntry *entry) { return *entry & 0x000FFFFFFFFFF000ULL; }