...
EFI_STATUS map_memory(uint64_t *pml4, efi_memmap_t *const memMap, elf_loadinfo_t *const loadInfo) {
  for (size_t i = 0; i < memMap->size / memMap->descSize; ++i) {
    ...
    EFI_TRY_RET(mem_map_pages(pml4, desc->PhysicalStart + DIRECT_MAP_BASE, desc->PhysicalStart, desc->NumberOfPages));
  }

  for (size_t i = 0; i < loadInfo->sectionCount; ++i) {
    elf_sectioninfo_t *section = (elf_sectioninfo_t *) ((char *) loadInfo->sections + i * sizeof(elf_sectioninfo_t));
